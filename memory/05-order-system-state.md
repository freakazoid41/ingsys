# Order Management System — Current State (2026-09-03)
...
> **2026-09-02 fixes: `OrderItemTable` rejected unlock + `OForm` still-rejected toast + `DList` grouping header full-length + `pickletable` header-cache + `DList` popup fix + `Document_files` join/old_versions. 2026-09-02 late: Tedarik login `/tedarik` orange 560×840 140px Gdz, unified orange 2FA (`loginSms` single file), `GDZ/ADM` rename + DB re-seed, `public/index.php` `adm ? ADM : GDZ`. 2026-09-02 late polished: TedarikPanel 6 tabs 64×12 protruding -52 (38px outside, 10px gap), logo 82px + label, menu flex1 centered, OList card-rows + PickleTable overrides, `pickletable/script.js` height auto guard. 2026-09-02 late: Module Switcher `Modüller` above Çıkış (Sidebar+TedarikPanel) → modal `Yönetim|Tedarik` `/coalpanel|/tedarikpanel`, Tedarik detailed search 3×3 hover `absolute top:52 z40` (9 fields, Şirket=client list `op-doc-client`/`lifnr`, Tedarikçi=users with client code) + `Documents::tableList` 9 keys (`stok_kodu/siparis_kodu/alim_kodu/seri_no/uretim_tarihi/sirket/tedarikci/onay_durumu/tarih_araligi`), shared pages `OList/OForm` via `isTedarik` keep-shared decision. 2026-09-02 late filter: `Filtreler` dropdown `teleport to body` `fixed` (9 radios + `Şirkete Göre Arama` → client table modal `720px` `8` rows lazy `200` `PickleTable` `flatpickr range` `Y-m-d` → `tarih_araligi` `i.created_at` + `to_date(entity_value)`), `Detaylı` twin inputs → same modal `single`, `OList.vue` `persons` fetch **killed** (was `POST /api/v1/table/persons` on mount), `Documents.php` `alim/siparis` ordering + `sirket/transactions` multi `IN` + `tarih_araligi` entity `to_date`. **FIX 2026-09-02 late: Şirkete Göre Ara empty → `handleDetayliChoice sirkete_gore` was `showClientModal=true` without `buildClientTable` → modal empty `Yükleniyor`; fixed to `openClientModal('multi')` + instant hardFallback in `openClientModal` + `PickleTable data: localData` scope fix → `this.modalClients.slice()`. Detaylı `Filtrele/Sıfırla` now auto-close `showDetailed=false`. OForm `isTedarik` 6-step fresh order detail `OForm.vue:966` header (ctitle/buying_no/order_no/`formatDate`) + warning +1 `OrderItemTable` +2 `tedarikDesc` +3 `tedarikImalatci` +4 `printMalzemeKabul/Cins` +5 `tedarikKabul/CinsFile` temp-upload +6 Gönder `formatDate` fix `OForm.vue:963`.*** **FIX 2026-09-02 late typewriter: `TedarikPanel.vue:551` holder-fixed/paper-feed — `root:fixed inset:0 overflow:hidden` + `frame:calc(100vh-40px) overflow:visible` + `main:overflow:visible bg:#fff` + `main-inner:height:auto bg:#fff will-change:transform translateY(-scrollY)` + `body height=scrollHeight+40` + `ResizeObserver` + `watch $route` + `OForm.vue:1463 tedarik-detail bg:#fff` — only `tedarik-main` travels, hidden at browser viewport, tabs+frame pinned, sub-cards not floating.** **THEME 2026-09-02 late tedarik items image 1:1: `OrderItemTable.vue:hideHeader` + `tedarik-thead` + `oic-row--tedarik` (Malzeme/K.Parti/Birimi/Miktar/Durum orange pill `Test Dokümanı Bekleniyor` + Detaylar gray) + serial/file white sections, header `#28` + `Sipariş Kalemleri` removed, wrapper border removed, `oic-hide-header` borderless.**
...
| `OrderItemTable.vue` | **DRUM STATUS 2026-09-02 late:** `OForm.vue:120 tedarikStatus` computed from `parsedStatus[last]` → `label/cls/icon` (`Beklemede`/`Kontrol`/`Sevke Hazır`/`Onaylandı`/`Reddedildi`) + `tedarik-detail-header` drum pill `tedarik-status-drum` `tedarik-status-dot` + waiting `#FF5A1F` ready `#fef3c7` approved `#dcfce7` rejected `#fee2e2`. |
| `OrderItemTable.vue` | **AT_ONCE COLLAPSED 2026-09-02 late:** `isCollapsed` → `if(hideHeader && atOnceMode && undefined) return true` — tedarik `Tek Parça` serial `Ürün parti...` default `Genişlet` shut, Excel upload forces `false`. |
| `OrderItemTable.vue` | **DETAYLAR MASTER 2026-09-02 late:** `tedarikDetailsCollapsed:{}` + `isTedarikDetailsCollapsed` + `toggleTedarikDetails` flips master wrapper `tedarik-additional is-collapsed/is-expanded max-height 3200px transition 0.38s` around files+serial+atOnce+split. Checkbox `toggleCard` when `hideHeader` → checking `tedarikDetailsCollapsed=false`, unchecking `tedarikDetailsCollapsed=true`. |
| `OrderItemTable.vue` | **Genişlet HIDDEN 2026-09-02 late:** `v-if="!hideHeader"` on all `Genişlet/Daralt` spans + `v-show="hideHeader?true:!is..."` for bodies + `oic-item-files-header{display:none}` (empty grey bar gone). |
...
| `OrderItemTable.vue` | **DRUM STATUS 2026-09-02 late:** `OForm.vue:120 tedarikStatus` computed from `parsedStatus[last]` → `label/cls/icon` (`Beklemede`/`Kontrol`/`Sevke Hazır`/`Onaylandı`/`Reddedildi`) + `tedarik-detail-header` drum pill `tedarik-status-drum` `12.5px 700` variants waiting `#FF5A1F` ready `#fef3c7` approved `#dcfce7` rejected `#fee2e2`. |
| `OrderItemTable.vue` | **AT_ONCE COLLAPSED 2026-09-02 late:** `isCollapsed` → `if(hideHeader && atOnceMode && undefined) return true` — tedarik `Tek Parça` serial `Ürün parti... (Toplam: 1560 KG)` default `Genişlet` shut, Excel upload forces `false`. |
| `OrderItemTable.vue` | **DETAYLAR MASTER 2026-09-02 late:** `tedarikDetailsCollapsed:{}` + `isTedarikDetailsCollapsed` + `toggleTedarikDetails` flips `tedarikDetailsCollapsed` + sync `itemFilesCollapsed`, `oic-row--tedarik` Detaylar `{{isTedarikDetailsCollapsed?'Detaylar':'Gizle'}}` + wrapper `<div :class="hideHeader?'tedarik-additional':''" v-show="hideHeader?!isTedarikDetailsCollapsed:true">` around all below-row (files+serial+atOnce+split) — whole drawer. |
