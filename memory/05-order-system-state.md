...
| `DList.vue` | **+Fix grouping (2026-08-31 late):** `groupFormatter` handles null/invalid JSON in `relation_detail`, falls back to `entity_tag` or "Belge" title. `rowFormatter` wrapped in try/catch, handles null `relation_detail`. Date column "Invalid Date" fixed — checks `dayjs.isValid()` before formatting. **+Fix grouping v2 (2026-08-31 late):** `groupFormatter` extracts `order_no` from `relation_detail` entities (not just `title`). `rowFormatter` now also sets raw field name from `entity_tag` (e.g. `order_no`) as top-level key. Date column type changed to `date`, added fallback to `rowData.created_at`. |
