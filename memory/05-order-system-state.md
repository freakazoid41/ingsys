...
| `OrderItemTable.vue` | **CHECKBOX AUTO 2026-09-02 late:**
...
| `OrderSystemSeeder.php` + `sys_options` + `OList.vue` | **Status label 2026-09-03:** doc_trans_order_transfer_sent Transfer Gönderildi→Dosyalar Kontrol Ediliyor, approved Sipariş Onaylandı→Kalite Onayı Verildi, DB UPDATE, OList labelMap fallback, tableList now doc_trans_order_transfer_sent**Dosyalar Kontrol Ediliyor for 3510004600-1. | `toggleCard` when `hideHeader` checking `!wasSelected` → `tedarikDetailsCollapsed=false` (opens whole), unchecking `wasSelected` → `delete split/serial` + `tedarikDetailsCollapsed=true` (closes whole) — checkbox opens/closes all sub areas, not just serial. |
