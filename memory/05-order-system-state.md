...
| `OrderItemTable.vue` | **CHECKBOX AUTO 2026-09-02 late:** `toggleCard` when `hideHeader` checking `!wasSelected` → `tedarikDetailsCollapsed=false` (opens whole), unchecking `wasSelected` → `delete split/serial` + `tedarikDetailsCollapsed=true` (closes whole) — checkbox opens/closes all sub areas, not just serial. |
