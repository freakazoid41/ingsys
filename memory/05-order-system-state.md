...
> **⚠️ 2026-09-04: Permission system rebuilt — 27 perms, 4 role templates, single login, module selection, OList pagination fixed. `per-08` and `per-04-05` removed. `canProceed` removed. Strict permission enforcement (cancelOrder→per-05-04, renameOrder→per-05-05, Kalite→per-05-03). Reseller LIFNR filter for orders. 2026-09-04 late: DList tedarik Filtreler (normal) added + double-table fix + Swal text-color fix + date-range ` - ` → `|` fix.**
