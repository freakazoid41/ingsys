...
> **⚠️ CURRENT STATE 2026-09-03: Order Management System + Public Tedarik Panel POLISHED + Tedarik Doküman LIVE.** DB `tedarikNewApp` :5431, 8 orders / 21 items / 8 clients / 7 files / 0 serials (2026-09-03 live, 7 files: 3×4400-1 +2×4600-2 +2×3500-1). Transfers = `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` **PURGED 2026-09-02** (6 rows). Order↔Client by `LIFNR`. Tenants **GDZ/ADM** via `GDZ.svg/ADM.svg`. Public login `/tedarik` → `/tedarikpanel` orange Gdz 560×840 → TedarikPanel 1360px 12px sidebar 210px 6 tabs 64×12 -52 (38px outside) + PickleTable 0 7px auto. **Shared pages `OList/OForm/DList` via `isTedarik` (keep shared)**, Module Switcher, Detailed search, File replacement FINAL + item files via `group_key`. **2026-09-03: Tedarik Doküman `/tedarikpanel/documents` LIVE** — `DList.vue isTedarik` flat list (grouping REMOVED expensive → `Sipariş Kodu` = `group_key` COALESCE same-conn/parent for test files), LIFNR-scoped `Document_files::tableList` for reseller, pagination `height:auto number` + height fixes (`pickletable height:auto`, Simplebar/typewriter content-driven, card `overflow:hidden` 16px). **FIX 2026-09-02 late: Şirkete Göre Ara hardFallback + showDetailed=false; OForm 6-step tedarik detail.** **FIX 2026-09-03: Aksiyonlar Kalite Onayı + -X suffix-only + Desc/Imalatci locked + 22P02 + file split + Dosyalar Kontrol Ediliyor; who/why unwrap + accepted visible + Gönder loading.**
