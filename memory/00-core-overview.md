...
> **⚠️ CURRENT STATE 2026-09-02 late: Order Management System + Public Tedarik Panel POLISHED.** DB `tedarikNewApp` :5431, 8 orders / 21 items / 8 clients / 0 files / 0 serials (re-seeded 2026-09-02 GDZ). Transfers = `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` **PURGED 2026-09-02** (6 rows deleted). Order↔Client by `LIFNR`. Tenants **GDZ/ADM** (ex CATES/YATAGAN) via `GDZ.svg/ADM.svg`. Public login `/tedarik` → `/tedarikpanel` orange Gdz card 560×840 140px logo → TedarikPanel card 1360px white `12px` shadow, sidebar 210px, 6 tabs 64×12 protruding -52 (38px outside) `48px` left gap, logo 82px + `Malzeme Tedarik İş Süreci` 11.5px, menu centered flex1, PickleTable card-rows `0 7px` `auto`. **Shared pages** `OList/OForm` via `isTedarik` (keep shared 2026-09-02), Module Switcher `Modüller` above Çıkış → `/coalpanel|/tedarikpanel`, Tedarik detailed search 3×3 hover `absolute` (Stok/Sipariş/Alım/Seri/Üretim/Şirket=client list/Tedarikçi=users with lifnr/Onay/Tarih). File replacement FINAL + item files (Test single + Görseller multi) via `group_key`. **FIX 2026-09-02 late: Şirkete Göre Ara empty → openClientModal multi + instant hardFallback + data scope fix, Detaylı Filtre/Sıfırla auto-close `showDetailed=false`. OForm `isTedarik` 6-step detail (`OForm.vue:966` screenshot 1:1 fresh order header+warning+1 Malzeme/tedarik tipi with `OrderItemTable` +2 Açıklama `tedarikDesc`+3 İmalatçı `tedarikImalatci`+4 İndir `printMalzemeKabul/Cins` +5 Yükle `tedarikKabul/CinsFile` temp-upload +6 Gönder `formatDate` fix `OForm.vue:963`).** **FIX 2026-09-03 late: who/why visible (getTedarikNote unwrap JSON) + accepted visible when other rejected (green lock) + Gönder isSubmitting loading. FIX 2026-09-03: Aksiyonlar Kalite Onayı (all files accepted) + Sipariş No -X suffix-only (base lock) + status pill disabled tedarik + ended guard + Desc/Imalatci locked after first save + SQL 22P02 temp fkey fix + file split admin mechanic (prefix like, reuse tag) 3510004600-2 repair + status Transfer→Dosyalar Kontrol. FIX 2026-09-02 late typewriter: `TedarikPanel.vue` holder-fixed/paper-feed — `root:fixed inset:0 overflow:hidden` + `frame:height calc(100vh-40px) overflow:visible` + `main:overflow:visible height:100% bg:#fff` + `main-inner:height:auto overflow:visible bg:#fff will-change:transform translateY(-window.scrollY)` + `body height = scrollHeight+40` + `ResizeObserver` + `watch $route` + `OForm.vue tedarik-detail bg:#fff` — only `tedarik-main` travels, hidden at browser viewport (not container), tabs+frame pinned, sub-cards not floating on grey.** **THEME 2026-09-02 late tedarik items: `OrderItemTable.vue:hideHeader` removes `Sipariş Kalemleri` header + wrapper, `oic-hide-header` borderless flush, `tedarik-thead` Malzeme Adı/K.Parti No/Birimi/S.Miktarı/Durum + `oic-row--tedarik` 12px gap orange `Test Dokümanı Bekleniyor` / green / red pills + gray `Detaylar`, serial/file sections white.** **FIX 2026-09-02 late tedarik polish: `hideHeader` removes `Sipariş Kalemleri`/`#28` + wrapper double border, `tedarik-thead` + `oic-row--tedarik` orange pill, drum `tedarikStatus` pill, at_once `isCollapsed` default true, `Detaylar` master `tedarikDetailsCollapsed` for whole additional, `Genişlet/Daralt` hidden `v-if="!hideHeader"` + file header `display:none` (empty grey bar gone), checkbox auto `toggleCard` opens/closes whole drawer.** **TEDARIK FORM POLISH 2026-09-02 late:** `isTransferLocked`/`tedarikDisplayMode` disables radio on locked, step 6 `v-if` new/rejected only, `parseTedarikExistingFiles` + `getTedarikDisplayName` + `previewTedarikFile` + status badges + lock note, `navigateToParentOrder`/`goBackToList`/`findAndNavigateToOrder` panel-aware routes (`TedarikOrderForm/TedarikOrderList` vs `OrderForm/OrderList`), header `← Tüm Siparişler` fancy pill + `office-bag` icon + `TDNO` badge + meta right-aligned + `tedarik-status-dot` + gradient top bar + `tedarik-detail-header` `radius 16px gradient shadow`.** Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).
