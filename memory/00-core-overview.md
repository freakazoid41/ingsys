...
> **⚠️ CURRENT STATE 2026-09-07 late5: CLIENT SYSTEM SPLIT `GDZ-0000300186` vs `ADM-0000300186` (`client_system` GDZ/ADM) — same numeric `lifnr` now duplicate across systems, `SyncOrders lifnr+system`, `Documents/Files LIFNR+SYSTEM` gates, `CList`+modals+`Bağlı Cariler` Sistem pill + composite `GDZ-...`, admin sees all `GDZ+ADM` (9). Prev late4: NOTIF `last_trans_at` (status time `05:57` tops, not birth `04:56`), `tedarik-03` `waiting,refreshed`, `tedarik-07` no `files_rejected` fallback, bulk `setFileStatusAll` per-order `tedarik-04/05`, `Test Dökümanı` `(!readonly||isTestRejected)` in `files_rejected` locked, Gmail `smtp.gmail.com` `kadir@`, modals `tdk/adm-notif-*` `460px`, `Bilgilendirmeler` `bind` fix, `TedarikDForm` redirect. Prev 2026-09-06 night: 8/21/8/0/37 `tedarikNewApp:5431` `GDZ` clean `/tmp/sap_fresh_payload.json` 21 rows → 8 EBELN. LOGGING FIX `Sipariş Dökümanı` + `doc_qnid` file timeline + serial enriched `Hilal→actor` + `0-qty` depleted lock `Tükendi` + file lock for `0` qty `Test/Görsel Ekle` hidden. OForm first-step Kabul/Cins, LIFNR fix, Aksiyonlar hide, Durum loading, dashboards polished.** DB `tedarikNewApp` :5431. `EBELN-X` clones, `op-doc-transfer` PURGED. `LIFNR+SYSTEM` composite link, `GDZ/ADM`. `/tedarik` → `/tedarikpanel` 1360px 12px 210px **7 tabs**. **Shared `OList/OForm/DList/DForm` via `isTedarik`**, Module Switcher, Detailed search, `group_key`. **`GET /api/v1/file-detail/:qnid` OLD-APP replica** + `LList` side-panel (`form_type` `Sipariş` vs `Sipariş Dökümanı` for files). `optimize-suggestions.md` has shared composables. **Admin Dashboard:** `adminStats|adminStatusBreakdown|adminRecentOrders|adminRecentFiles|adminActivity|adminMonthlyOrders` Cache 60s + 8 Vue + flex fix + `OList ?status=` . **Tedarik Dashboard:** `tedarikStats 6 (totalItems fix) + tedarikMonthly|tedarikRecentFiles|tedarikActivity Bilgilendirmeler` + 7 Vue + `DForm admin-theme blue`.
