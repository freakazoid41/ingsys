...
> **⚠️ CURRENT STATE 2026-09-01: CONVERTED to Order Management System.** DB `tedarikNewApp` :5431, 8 orders / 21 items / 8 clients / 0 files / 0 serials (fresh sync `/tmp/sap_payload.json` → 8 EBELN grouped, mixed ST/KG/M, all ST<300). Transfers = `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` unused. Order↔Client by `LIFNR`. Lock: `ready_for_shipment` FULL, `files_rejected` keeps files editable. File replacement FINAL design + item files (Test Dökümanı rejectable + Ürün Görselleri multi) per order item. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).
