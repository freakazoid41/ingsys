...
> **⚠️ CURRENT STATE 2026-09-06 late: 10/24/8/4/37 `tedarikNewApp:5431` `GDZ` (2 clones). OForm first-step required Kabul/Cins (canSend+files_rejected) + Documents LIFNR fix for items/serials (kbbozat41 now sees 4 kalem) + Tedarik Aksiyonlar hide if no per-05-03/04/05 (Detaylar stays) + Durum loading/updateRow + Admin/Tedarik dashboards polished + wrapper padding fix. Prev 2026-09-06: Admin/Tedarik REBUILT + DForm blue + Stat→List + flex fix + Caching; 2026-09-05: OList/DList purge + composables + backend-only + YYYY/MM + IS_TEST + kbbozat41 YILDIZ.** DB `tedarikNewApp` :5431. `EBELN-X` clones, `op-doc-transfer` PURGED. `LIFNR` link, `GDZ/ADM`. `/tedarik` → `/tedarikpanel` 1360px 12px 210px **7 tabs**. **Shared `OList/OForm/DList/DForm` via `isTedarik`**, Module Switcher, Detailed search, `group_key`. **`GET /api/v1/file-detail/:qnid` OLD-APP replica** + `LList` side-panel. `optimize-suggestions.md` has shared composables. **Admin Dashboard:** `adminStats|adminStatusBreakdown|adminRecentOrders|adminRecentFiles|adminActivity|adminMonthlyOrders` Cache 60s + 8 Vue + flex fix + `OList ?status=` . **Tedarik Dashboard:** `tedarikStats 6 (totalItems fix) + tedarikMonthly|tedarikRecentFiles|tedarikActivity Bilgilendirmeler` + 7 Vue + `DForm admin-theme blue`.
