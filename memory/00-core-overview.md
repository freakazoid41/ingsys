...
> **⚠️ CURRENT STATE 2026-08-31: CONVERTED to Order Management System — DB `tedarikNewApp` (5431) docker `tedarikNewApp` Up. Transfers are `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` NOT used. Order↔Client by `LIFNR`. 8 orders (5 SAP fresh 3510004200-4600 + 3 original) 23 items. Order detail: items ROW list, transfer card (Tek Seferde/Parçalı) when `canSend`, Form below. **Lock: `ready_for_shipment` FULL lock (desc+imalatci+files). `files_rejected` keeps files editable. NEW RULES 2026-08-31: (1) Once partitioned → only Parçalı forever (`hasActivePartitions` guard in `DocumentServiceProvider::processOrderTransfer` + `OForm::checkHasPartitions` disables Tek Seferde), all clones removed → Tek Seferde unlocks. (2) Removing a clone (`Parçayı Sil` via `cancelOrder/removeContent`) restores its `split_amount` to main via `incrementItemQuantity`/`restoreQuantitiesForClone`. (3) Malzeme Kabul PDF: `imalatci_firma_adi` now inside signature box (not floating).** File replacement **FIXED**. See `memory/05-order-system-state.md` (LIVE), `memory/06-roadmap-next.md`. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).**
