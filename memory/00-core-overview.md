...
> **⚠️ CURRENT STATE 2026-08-26: CONVERTED to Order Management System — DB `tedarikNewApp` (5431) docker `tedarikNewApp` Up. Transfers are `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` type NOT used. Order↔Client linked by `LIFNR` (`Cari Kodu`). 1 test order `3510001793` + 1 item `parent_id`. Order detail shows `OrderItemTable` + textarea desc + Malzeme Kabul/Cins files. **Transfer flow BUILT (2026-08-26): order-detail SAVE carries `transfer_mode` (`at_once`/`partial`) + `selected_items` → `processOrderTransfer` (at_once sends order; partial clones `EBELN-X` + duplicates items + moves files). New status `doc_trans_order_files_rejected` (Reddedilen Dosyalar Mevcut) auto-set when any file rejected (`syncOrderStatusFromFiles`). `POST /v1/orders/cancel` rejects & cancels whole order. Locking via Form.vue `readonlyFields`. Status guard `created→transfer_sent→approved/rejected|files_rejected→transfer_sent`. Front panel design PENDING, SAP cron PENDING (grouping defined).** See `memory/05-order-system-state.md` (LIVE), `memory/06-roadmap-next.md`. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).**
