...
> **⚠️ CURRENT STATE 2026-08-27: CONVERTED to Order Management System — DB `tedarikNewApp` (5431) docker `tedarikNewApp` Up. Transfers are `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` type NOT used. Order↔Client linked by `LIFNR` (`Cari Kodu`). 1 test order `3510001793` + 2 items `parent_id`. Order detail: items table ALWAYS at top, transfer selection (at_once/partial) above it when canSend, Form below. **Transfer flow BUILT: order-detail SAVE carries `transfer_mode` + `selected_items` → `processOrderTransfer` (at_once sends order; partial clones `EBELN-X` + duplicates items and moves order/item files). `syncOrderStatusFromFiles` stops at the nearest order, including clones. Status flow is `created→transfer_sent→ready_for_shipment (Sipariş Sevke Hazır)→approved/rejected`, with `files_rejected` when an active current file is rejected.** `$key` clobbering bug FIXED in DocumentController PUT (foreach loop). Textarea `readonlyFields` support added. File replacement mechanic is STABLE (do not change without approval). Order status badges use related icons.** See `memory/05-order-system-state.md` (LIVE), `memory/06-roadmap-next.md`. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).**
