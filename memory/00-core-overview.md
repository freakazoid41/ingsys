...
> **⚠️ CURRENT STATE 2026-08-28: CONVERTED to Order Management System — DB `tedarikNewApp` (5431) docker `tedarikNewApp` Up. Transfers are `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` type NOT used. Order↔Client linked by `LIFNR` (`Cari Kodu`). 1 test order `3510001793` + 2 items `parent_id`. Order detail: items ROW list (cards→rows, max-h 420px scroll, 10-15 rows) ALWAYS at top, transfer selection (at_once/partial) above it when canSend, Form below (Malzeme Kabul/Cins single-file `hideAdd:true`). **Transfer flow BUILT: order-detail SAVE carries `transfer_mode` + `selected_items` → `processOrderTransfer` (at_once sends order; partial clones `EBELN-X`). `syncOrderStatusFromFiles` stops at nearest order. Status flow `created→transfer_sent→ready_for_shipment (Sipariş Sevke Hazır)→approved/rejected` + `files_rejected`. Lock: `OForm.vue:63` `lockedStatuses` includes `ready_for_shipment`, `isFilesLocked` locks `transfer_kabul/cins` files also (except `files_rejected` where files stay editable for replace), `Form.vue:2385` handles compound `**` names. `$key` clobber FIXED. File replacement STABLE.** See `memory/05-order-system-state.md` (LIVE), `memory/06-roadmap-next.md`. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).**
