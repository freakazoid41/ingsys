...
> **⚠️ CURRENT STATE 2026-09-06: 10/24/8/4/37 `tedarikNewApp:5431` `GDZ` (2 clones 4400-1/4600-1, 4 files). Admin Dashboard REBUILT + Tedarik Dashboard POLISHED + File-detail admin blue theme + Stat→List filtering + Sidebar flex fix + Caching. Prev 2026-09-05: OList/DList purge + shared composables + Şirket backend-only + uretim YYYY/MM + IS_TEST 111111 + kbbozat41 YILDIZ.** DB `tedarikNewApp` :5431. `EBELN-X` clones, `op-doc-transfer` PURGED. `LIFNR` link, `GDZ/ADM`. `/tedarik` → `/tedarikpanel` 1360px 12px 210px **7 tabs**. **Shared `OList/OForm/DList/DForm` via `isTedarik`**, Module Switcher, Detailed search, `group_key`. **`GET /api/v1/file-detail/:qnid` OLD-APP replica** + `LList` side-panel. `optimize-suggestions.md` has shared composables. **Admin Dashboard:** `ReportServiceProvider::adminStats|adminStatusBreakdown|adminRecentOrders|adminRecentFiles|adminActivity|adminMonthlyOrders` (Cache 60s, global) + 8 Vue (Header/Stats 8 cards/StatusChart/MonthlyChart/RecentOrders/Files/Activity/QuickActions) + `Admin.vue` flex fix `aside:relative` scoped + `OList ?status=` filtering. **Tedarik Dashboard:** `tedarikStats (6 cards, totalItems fix parent d2 spec_code) + tedarikMonthlyOrders|tedarikRecentFiles|tedarikActivity (Bilgilendirmeler, only doc_trans_order_%/doc_file_%)` + 7 Vue (Header/Stats 6/Status/Monthly/RecentOrders/Files/Activity/QuickActions 4) + `DForm admin-theme blue`.
