...
> **⚠️ CURRENT STATE 2026-09-01: CONVERTED to Order Management System.** DB `tedarikNewApp` :5431, 10 orders / 23 items / 8 clients / 10 files / 2 serials. Transfers = `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` unused. Order↔Client by `LIFNR`. Lock: `ready_for_shipment` FULL, `files_rejected` keeps files editable. File replacement FINAL design + item files (Test Dökümanı rejectable + Ürün Görselleri multi) per order item. DList groups all files under clone order number via `group_key` SQL column. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).
