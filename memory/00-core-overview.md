...
> **⚠️ CURRENT STATE 2026-08-31: CONVERTED to Order Management System — DB `tedarikNewApp` (5431) docker `tedarikNewApp` Up. Transfers are `op-doc-order` clones (`EBELN-X`), `op-doc-transfer` NOT used. Order↔Client by `LIFNR`. 8 orders 23 items (fresh sync, all ST < 300, mixed ST/KG/M, 0 files). Order detail: items ROW list, transfer card (Tek Seferde/Parçalı) when `canSend`, Form below. **Lock: `ready_for_shipment` FULL lock (desc+imalatci+files). `files_rejected` keeps files editable + print button. NEW RULES 2026-08-31: (1) Once partitioned → only Parçalı forever (`hasActivePartitions` guard in `DocumentServiceProvider::processOrderTransfer` + `OForm::checkHasPartitions` disables Tek Seferde), all clones removed → Tek Seferde unlocks. (2) Removing a clone (`Parçayı Sil` via `cancelOrder/removeContent`) restores its `split_amount` to main via `incrementItemQuantity`/`restoreQuantitiesForClone`. (3) Malzeme Kabul PDF: `imalatci_firma_adi` now inside signature box (not floating). (4) Serial UI: at_once + partial both have Daralt/Genişlet collapse, partial bg yellow/amber, at_once bg gray, Daralt far right. (5) Reprint: `files_rejected` shows print button in locked card, uses DB data. Clone order_no stays as-is for reprints. (6) Clone stores `transfer_mode` in EAV.** KG/M auto serial date: pre-filled from order `created_at`, not required.** File replacement **FIXED** (entity rows per version + `document_files.status` activeness + `replaced_id` backward + status array `ORDER BY t.id`; porting doc `file-versioning-system.md`). `syncOrderStatusFromFiles` **FIXED** (fires from registerContent + transfer_sent branch).** See `memory/05-order-system-state.md` (LIVE), `memory/06-roadmap-next.md`. Panel `http://127.0.0.1:8000` (`kadir@kontent.com.tr / Kadir412. / 111111`).**
