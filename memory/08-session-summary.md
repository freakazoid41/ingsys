...
- **DList detail popup İlişki & Invalid Date + Document_files old_versions (2026-09-02):** `rowFormatter` overwrote `data.created_at` with EAV `created_at="20/08/2026"` (order SAP `d/m/Y`) → popup `dayjs("20/08/2026")` = `Invalid Date` (table used `_created_at_fmt` before overwrite, so table was fine but modal not). Fix: preserve `_file_created_at`/`_created_at_fmt` before loop, skip overwriting `created_at/id/file/qnid`. Popup now `product_name||title||order_no` for İlişki (orders have no title → showed blank → now `order_no 3510003500-1`) + `_created_at_fmt` with `isValid()` fallback. `Document_files.php` join `+ se.table_tag='document_files'` + `old_versions` `df2.id::text=se2.entity_value and se2.table_tag='document_files' and se2.entity_value ~ '^[0-9]+$'` — garbage `quantity=180` duplicate row no longer links, no more `340.60` cast crash.
