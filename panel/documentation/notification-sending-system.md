...
### 7.3 Re-upload after rejection now fires `tedarik-03` `DocumentServiceProvider.php:270`
* **Bug:** `tedarik-03` `İnceleme Bekleyen Dosyalar Mevcut` was only sent on *initial* transfer `DocumentController.php:206` `processOrderTransfer` → `sendTedarikOrderSent` + `sendTedarikFileWaiting` together (`fileTitle='Transfer dosyaları'` BUKRS). When tedarik re-uploaded a *rejected* file (`doc_file_rejected` → `doc_file_refreshed` via `finalizeTempFile:599` / `addFileToDb:842`), `ReportServiceProvider.php:137` correctly listed it as `waiting,refreshed`, but `EmailServiceProvider:115` was never called → assigned `tedarik-03` admins got no mail, bell `SystemController:95` showed item but mail silent.
* **Fix:** `DocumentServiceProvider.php:270` `registerContent` now tracks `$refreshedFromRejected` (checks `transactions op_id=1 last op_key === 'doc_file_rejected'` for any `existingFileId>0`). After `DB::commit` + `syncOrderStatusFromFiles` (which recovers `files_rejected → transfer_sent`), if flag true and `empty(transfer_mode)` (file edit, not initial send) resolves orderDoc via `lastFileEntity->conn_id → main_id → parent fallback` → `orderSnapshot` → `sys_code/bukrs` → `EmailServiceProvider::sendTedarikFileWaiting(['order_no','sys_code','qnid','fileTitle'=>'Yenilenen dosya'])` BUKRS-gated `SendNotificationMailJob:506` `tedarik-03`. One mail per `PUT /v1/document` save, not per file, matches `tedarik-02` single-dispatch pattern. Initial send still via `DocumentController:179` (not duplicated because `transfer_mode` present skips this path).
