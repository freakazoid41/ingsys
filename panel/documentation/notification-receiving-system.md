...
### 10.1 `order by id desc` → `order by status change` (newest first)
* **Backend `ReportServiceProvider.php:181,224,280,291` + `Documents.php:83` `last_trans_at` + `Document_files.php:95` `last_trans_at` + `last_status.created_at`** — per-category `usort` now `last_trans_at || last_status.created_at || created_at DESC` primary → `main_id` fallback (was `created_at` = `i.created_at` birth, so `3510004400 04:56:38` birth topped `3510004400-1 05:56:48` birth even though order's `doc_trans_order_approved` at `05:57:15` is after file `doc_file_accepted 05:57:04`). Explicit `order=>main_id desc` still forced. After BUKRS/read, `array_slice` keeps newest 10 by status time.
* **Frontend `notificationHelpers.js:7` `parseOrder/File`** — `statusAt = last_trans_at || last_status.created_at || created_at`, `makeRow created_at = statusAt`, `_created_at_fmt = fmtDateTime(statusAt)`. Sort `created_at` = status time.
* **Headers** `TedarikHeader.vue:79` `parseOrder/File` same `last_trans_at` extraction, `DashboardHeader.vue:106` + `coalparts/Header.vue:129` same, `sortByNewest rawTime` now status time so `558 approved 05:57:15` tops `557 05:57:04` correctly. Previous `04:56 Kalite` below `05:44/05:56` files fixed.

### 10.2 Modal redesign — no overlap, no clipped rows
* **Tedarik** `TedarikHeader.vue:260` `tdk-notif-*` orange `460px` `fff→fff7ed` header `44px` gradient icon, body `360px #fcfcfe` **block + margin-bottom 9px** (not flex `gap`) + `border-left 3px` (not `::before` stub), icon `38px 10px` per-type `tedarik-01 #fff7ed/#9a3412` etc, `title 13px 700` + `meta 11.5px #64748b ki-time` (status time `last_trans_at`), arrow `28px` hover `FF5A1F`. Previous `gap:9px` collapsed inside Swal `.htmlContainer` + `::before` `top0/bottom0` rendered as 8px stub → rows overlapped, pills duplicated. `Bilgilendirmeler.vue:111` arrow `bind` fix also covered here.
* **Admin** `coalparts/Header.vue` + `DashboardHeader.vue:390` `adm-notif-*` navy `154B91` same cards `11.5px meta` (status time), `eff6ff` icons, `f8fafc` body `cbd5e1` scrollbar. Both use `customClass: adm/tdk-notif-popup (18px radius, 0 padding, 0 24px 64px shadow)`.

### 10.2b `Test Dökümanı` rejected slot — hidden upload
* **`OrderItemTable.vue:1325`** rejected chip `Reddedildi` had `v-if="!readonly && !isDepleted"` for `Yeni Test Dökümanı Yükle` — when order `files_rejected` → `OForm :readonly=isLocked` true, button hid even though `isTestRejected:811` true and `triggerTestUpload:817` allowed JS. Fixed to `(!readonly || isTestRejected(row.id)) && !isDepleted` + fallback lock text, so `af59e4e6 Maden Drogi 2` ` -MzhGOs… Reddedildi` now shows upload.
