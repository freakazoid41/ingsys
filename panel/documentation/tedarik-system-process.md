...
- Green `Malzeme Kabul Formu Yazdır` + purple `Malzeme Cins-Miktar Kabul Formu Yazdır` `printMalzemeKabul/Cins` `OForm.vue:753` `.print-cins-btn #7c3aed→#6d28d9`, also in locked card when `canPrintKabul = canSend || files_rejected` (reprint from rejected uses `effectiveTransferMode = canSend ? transferMode : storedTransferMode`, order_no stays suffix if already `EBELN-X`, items from `itemTable.items` when `selectedItems` empty).
...
- **⚠️ 2026-09-07 late15 FIX — `imalatci_firma_adi` print trap `OForm.vue:759` + `OForm.vue:803`:** until `ed18d08` both prints did `getFieldValue('imalatci_firma_adi') || orderEntities.imalatci_firma_adi`. Tedarik Step 3 input is standalone `v-model="tedarikImalatci"` `OForm.vue:1103` (no `name`, hidden `<Form ref=formRef style="display:none">`). `getFieldValue:952` only looks inside hidden `Form` + DOM `input[name^="imalatci"]` → always `''` on fresh order + `orderEntities` empty → `validatePrintForm:24 !imalatci` → `İmalatçı Firma Boş` even though user typed `Test İmalatçı`. Worked before `cab9f8a 2026-09-02 14:48` when `Form` was visible with `name="imalatci_firma_adi"` → DOM query succeeded; broke when tedarik split detached the field. Fix: `const imalatci = (tedarikImalatci?.trim()) || getFieldValue() || orderEntities` + same for `tedarikDesc` (both prints). Admin path unaffected (`tedarikImalatci=''` → falls through). **Rule:** any tedarik `v-model` detached from EAV must be explicitly preferred before `getFieldValue()`. `submitForm:407` already did manual `dynamicF inject` — print forgot.
...
- **Tedarik detached `v-model` trap `OForm.vue:759`:** any field moved out of `<Form>` (like `tedarikImalatci`/`tedarikDesc` `v-model`) is invisible to `getFieldValue()`. Always do `(tedarikX?.trim()) || getFieldValue('x') || orderEntities.x`. Check git `cab9f8a→ed18d08` — 15 commits broken. `submitForm` manual inject is not enough for print preview paths. Add this to PR checklist: `grep v-model.*tedarik` → verify every print/validate reads it first.
- **Tedarik-03 silence on refresh `DocumentServiceProvider.php:270`:** `tedarik-03` was only sent on initial `processOrderTransfer` `DocumentController:206` together with `tedarik-02`. Re-uploading a rejected file (`doc_file_refreshed` `DocumentHelpers:599` via `registerContent` `finalizeTempFile`) never dispatched `EmailServiceProvider:115` → `ReportServiceProvider:137` showed waiting but mail silent, `SystemController:95` bell wrong. Fix now tracks `$refreshedFromRejected` (`transactions last op_key === doc_file_rejected` check) and after `commit` + `syncOrderStatus` sends `sendTedarikFileWaiting` once per save with `BUKRS` via `orderSnapshot` parent fallback. Skip when `transfer_mode` present (initial send already covers). Check: `grep -n sendTedarikFileWaiting` must show both `DocumentController:206` and `DocumentServiceProvider:270`.
- **2026-09-08 late01: `free`/`all` `last_trans_at` broke `hasPartitions` lock `Documents.php:543` + `500` `DocumentController.php:1089` `listOrderFiles` `WHERE ((d.id=? OR d.parent_id=?))` leaked clone `3510004400-1` into base `84428217` `4` files + `all` alias `explode('as  ')` single vs double broke `last_trans_at`. Fix: `WHERE ((d.id=? AND dt.op_key='op-doc-order') OR (d.parent_id=? AND dt.op_key='op-doc-order-item'))` + `JOIN dt` + `strripos` alias + `DocumentController:1089` missing `))` → `500 ORDER BY` fixed; `DocumentServiceProvider.php:1312` `sync` same. `OForm.vue:151` `hasAnyFile` now `false` when empty → admin bottom `Bütün Formları` `v-if="hasAnyFile"` hidden for base `0` (clone `3`).**
- **2026-09-08 late02: Save loading lame + Detail Kalite `OForm.vue:1334` `isSubmitting` was `opacity .72` + `6366f1` line. Now gradient `ff7a3a→FF5A1F` + progress `tedarikProgress` + card `fff7ed/fed7aa` `28px` spinner; `canKalite` (`per-07-02` && not approved/rejected) + `kaliteOnayi()` `POST doc_trans_order_approved` + `acceptAll` + refresh; `OList.vue:828` same `Swal` confirm added.**
