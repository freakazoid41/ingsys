    ...
    mergeNotifications() {
      ...
      try {
        ...
        const parseOrder = (o) => {
          ...
          return { orderNo: orderNo||o.qnid||o.id||'-', ctitle, qnid: o.qnid||o.id||o.relation_qnid, main_id:o.main_id||0, created_at:statusAt, rawCreated:o.created_at||'', last_trans_at:o.last_trans_at||'' };
        };
        const parseFile = (f) => { let sAt = f.last_trans_at || ''; if(!sAt && f.last_status){ try{ const j=typeof f.last_status==='string'?JSON.parse(f.last_status):f.last_status; sAt=j.created_at||''; }catch{} } sAt = sAt || f.created_at || ''; return { orderNo: f.group_key||'-', title: f.type_title||f.file_type||'Dosya', qnid: f.qnid||f.id||f.relation_qnid, main_id:f.main_id||0, created_at:sAt, rawCreated:f.created_at||'', last_trans_at:f.last_trans_at||sAt }; };
        for (const key in addNotifications) {
          switch (key) {
            case 'orderImported':
              list = [...list, ...(addNotifications[key]||[]).map(o=>{ const m=parseOrder(o); const sub=m.ctitle; const at=m.created_at; return { id:`ted01-${m.qnid}`, text: `Sipariş Sisteme Geldi (SAP) — ${m.orderNo}${sub?' — '+sub:''}`, sub, orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik01', cat:'tedarik-01', opKey:'tedarik-01', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-package', onclick:()=>this.$router.push({name:'OrderForm',params:{id:m.qnid}}) }; })];
              break;
            case 'orderSent':
              list = [...list, ...(addNotifications[key]||[]).map(o=>{ const m=parseOrder(o); const at=m.created_at; return { id:`ted02-${m.qnid}`, text: `Sipariş Onaya Gönderildi — ${m.orderNo}`, sub:m.ctitle||'', orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik02', cat:'tedarik-02', opKey:'tedarik-02', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-send', onclick:()=>this.$router.push({name:'OrderForm',params:{id:m.qnid}}) }; })];
              break;
            case 'pendingFiles':
              list = [...list, ...(addNotifications[key]||[]).map(f=>{ const m=parseFile(f); const at=m.created_at; return { id:`ted03-${m.qnid}`, text: `İnceleme Bekleyen Dosyalar — ${m.title} (${m.orderNo})`, sub:`${m.title} • ${m.orderNo}`, orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik03', cat:'tedarik-03', opKey:'tedarik-03', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-file', onclick:()=>this.$router.push({name:'DList'}) }; })];
              break;
            case 'fileApproved':
              list = [...list, ...(addNotifications[key]||[]).map(f=>{ const m=parseFile(f); const at=m.created_at; return { id:`ted04-${m.qnid}`, text: `Sipariş Dosyası Onaylandı — ${m.title} (${m.orderNo})`, sub:`${m.title} • ${m.orderNo}`, orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik04', cat:'tedarik-04', opKey:'tedarik-04', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-check', onclick:()=>this.$router.push({name:'DForm',params:{id:m.qnid}}) }; })];
              break;
            case 'fileRejected':
              list = [...list, ...(addNotifications[key]||[]).map(f=>{ const m=parseFile(f); const at=m.created_at; return { id:`ted05-${m.qnid}`, text: `Sipariş Dosyası Yeniden Talep — ${m.title} (${m.orderNo})`, sub:`${m.title} • ${m.orderNo}`, orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik05', cat:'tedarik-05', opKey:'tedarik-05', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-cross', onclick:()=>this.$router.push({name:'DForm',params:{id:m.qnid}}) }; })];
              break;
            case 'orderApproved':
              list = [...list, ...(addNotifications[key]||[]).map(o=>{ const m=parseOrder(o); const at=m.created_at; return { id:`ted06-${m.qnid}`, text: `Sipariş Kalite Onayı Verildi — ${m.orderNo}`, sub:m.ctitle||'', orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik06', cat:'tedarik-06', opKey:'tedarik-06', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-medal-star', onclick:()=>this.$router.push({name:'OrderForm',params:{id:m.qnid}}) }; })];
              break;
            case 'orderRejected':
              list = [...list, ...(addNotifications[key]||[]).map(o=>{ const m=parseOrder(o); const at=m.created_at; return { id:`ted07-${m.qnid}`, text: `Sipariş Reddedildi — ${m.orderNo}`, sub:m.ctitle||'', orderNo:m.orderNo, time: `Kayıt: ${at||''}`, fmtTime: at?this.fmtTime(at):'', rawTime:at||'', rawId:m.main_id, type: 'tedarik07', cat:'tedarik-07', opKey:'tedarik-07', targetQnid:m.qnid, qnid:m.qnid, main_id:m.main_id, created_at:at||'', last_trans_at:m.last_trans_at||at, iconClass: 'ki-outline ki-cross-square', onclick:()=>this.$router.push({name:'OrderForm',params:{id:m.qnid}}) }; })];
              break;
            default:
              break;
          }
