<style scoped>
    * {
    ...
    .fab {
        ...
        border-radius: 50%;
        background: #edd9e4;
        box-shadow: 0px 5px 20px #edd9e4;
    ...
    .fab .fa{
        color: #B22774 !important;
    }

    .fab:hover span.fa{
        color: #edd9e4 !important;
    }
    .fab:before {
        content: "";
        position: absolute;
    ...
    .fab:hover {
        ...
        background: #B22774;
        box-shadow: 0px 5px 20px 5px #edd9e4;
    }
    ...
    methods: {
        async execute() {
            ...
            if (this.callback) {
                ...
                const rsp = await this.callback();
                //console.log(rsp)
                if(!rsp) this.status = 'await';
            }
        }
    }
  }
...
<template>
    <div class="fab-wrapper">
        ...
        <label class="fab d-flex justify-content-center align-items-center" v-if="btntype==='saveBtn'" for="fabCheckbox" @click="execute">
            <span class="fa fa-check fs-1" v-if="status=='await'"></span>
            <span class="fa fa-compact-disc fs-1 fa-spin" v-if="status=='loading'"></span>
        </label>
