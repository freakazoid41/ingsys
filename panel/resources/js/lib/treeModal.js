...
function normalizeItems(items, options) {
  ...
  if (items[0][parentKey] !== undefined && items[0][idKey] !== undefined) {
    const flatified = buildTreeFromFlat(items, idKey, parentKey, childrenKey);
    return normalizeItems(flatified, options); // ensures ids and children key are consistent
  }
...
function ensureArray(a) { return Array.isArray(a) ? a : []; }
  ...
  function render(options){
    ...
    const checkedSet = new Set(checkedList.flatMap(v => {
      if (v == null) return [];
      if (typeof v === 'object') {
        const mapped = [];
  ...
  function show(options){
    ...
    return new Promise((resolve)=>{
      try{
        ...
        const defaultChecked = new Set(normalized.map(String));
