<script>
    ...
    export default {
        breadcrumbs: {
            list: [ { title: 'Parking Spots', path: '/coalpanel/parking-spots' } ],
            title: 'Parking Spots'
        },
        setup() {
        ...
        methods: {
            ...
            buildTable(){
                const headers = [
                    { title : 'Spot', key : 'title', order : true, type : 'string' },
                    { title : 'Number', key : 'spot_number', order : true, type : 'string' },
                    { title : 'Level', key : 'level', order : true, type : 'string' },
                    { title : 'Section', key : 'section', order : true, type : 'string' },
                    { title : 'Size', key : 'size', order : true, type : 'string' },
                    ...
                    { title : 'Actions', key : 'id', order : false, type : 'string', columnFormatter : (elm,rowData,columnData) => {
                            ...
                            editBtn.onclick = () => this.$router.push({ name: 'ParkingSpotForm', params: { id: columnData } });
        ...
        <div class="card-header rlist-header">
            <div class="rlist-search-group">
                <div class="rlist-search-wrap">
                    ...
                    <input type="text" id="mainSearch" class="rlist-search-input" placeholder="Search parking spots...">
                </div>
            ...
            <div class="rlist-toolbar">
                <router-link :to="{ name: 'ParkingSpotForm' }" class="rlist-btn rlist-btn-create">
                    <i class="ki-outline ki-plus fs-5"></i> Add Spot
                </router-link>
            </div>
        </div>
