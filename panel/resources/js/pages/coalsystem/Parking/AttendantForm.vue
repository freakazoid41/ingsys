<script>
    ...
    export default {
        breadcrumbs: {
            list: [ { title: 'Attendants', path: '/coalpanel/attendants' }, { title: 'Attendant Add / Edit', path: '#' } ],
            title: 'Attendant Add / Edit'
        },
        components: { Form },
        setup() {
            return {
        ...
        methods: {
            async submitForm(formData){
                ...
                this.navigationStore.toggle(true);
                this.formData.typeKey = 'op-doc-attendant';
                ...
                if(rsp.valid){
                    ...
                    setTimeout(() => {
                        ...
                        this.plib.toast(this.Swal, response.success ? 'success' : 'error', response.msg || 'Operation completed', () => {
                            this.$router.push({ name: 'AttendantList' });
                        });
                    }, 300);
                } else {
                    this.navigationStore.toggle(false);
...
<template>
    <Form formtypes="op-doc-attendant-form" v-if="loadForm" :savecallback="submitForm" savebtntitle="Save Attendant" />
</template>
