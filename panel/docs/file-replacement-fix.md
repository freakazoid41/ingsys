...
| `panel/app/Providers/DocumentServiceProvider.php:228-247` | `registerContent` — looks up existing file entity (`table_tag='document_files'`) BEFORE processing. Extracts `$existingFileId` from old `entity_value`. Passes it to `finalizeTempFile`. |
