...
class Documents extends Model
{
    ...
    static function tableList($obj){
            ...
            switch($obj['order']['key']){
                default:
                    if(isset($columns[$obj['order']['key']])){
                        ...
                        $select[] = " MAX(CASE WHEN se.entity_tag = '".$obj['order']['key']."' THEN se.entity_value END) OVER (PARTITION BY i.id) AS name_value "; 
                        $order   = " ORDER BY fv.name_value ".$obj['order']['style'].' ';
                    }
                break;
            }
            
        }else{
            $order = ' order by fv.created_at desc ';
        }
        ...
        if (isset($obj['filter'])){
            ...
            if(isset($obj['filterKeys']['form-type'])){
                if(env('DB_CONNECTION') == 'sqlsrv'){
                    $outherApply = "OUTER APPLY (
                                        SELECT 
                                           CONCAT(
                                            '[', 
                                            STRING_AGG(
                                                CONCAT('{\"Key\":\"', se2.entity_tag, '\",\"Value\":\"', se2.entity_value, '\"}'), ','
                                            ),
                                            ']'
                                        ) AS main_attr_json
                                        FROM sys_con_ops so2
                                        INNER JOIN sys_con_entities se2 ON se2.conn_id = so2.id
                                        INNER JOIN sys_options sp2 ON sp2.id = so2.type_id
                                        WHERE so2.conn_id = 0 
                                        AND sp2.op_key = '".$obj['filterKeys']['form-type']."'
                                        AND so2.main_id = fv.realId
                                        GROUP BY so2.main_id
                                    ) main_attr";
                }else{
                    $columns['main_attr'] = "(SELECT    ".( env('DB_CONNECTION') == 'pgsql' ? 'json_agg' : 'json_group_array' )."(
                                                                ".(env('DB_CONNECTION') == 'pgsql' ? 'json_build_object' : 'json_object')."(
                            ...
                            $outherApply = "OUTER APPLY (
                                        SELECT 
                                            CONCAT(
                                                    '[',STRING_AGG(
                                                CONCAT(
                                                    '{\"Key\":\"', se2.entity_tag, '\",\"Value\":\"', se2.entity_value, '\"}'
                                                ), ','
                                            ) , ']') AS main_attr_json
                                        FROM sys_con_ops so2
                                        INNER JOIN sys_con_entities se2 ON se2.conn_id = so2.id
                        ...
                        if($value != ''){
                            ...
                            foreach($columns as $k=>$v){
                                ...
                                if(env('DB_CONNECTION') == 'pgsql'){
                                    ...
                                    $where .= " $column like '%$value%' or se.entity_value like '%$value%' ";
                                }
        ...
        //create query    
        $sql = "select fv.*,COALESCE(
                                main_attr.main_attr_json,
                                '[]'
                            ) AS main_attr
                from    (
                        SELECT 
                           distinct  ".implode(",", array_values($columns)).(!empty($select) ? ',': ' ').implode(",", $select).'
                        FROM documents i
                            '.$join.'
                            '.$where.$authWhere.'
                    ) AS fv
